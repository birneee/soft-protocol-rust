...
impl ConnectionPool {
    ...
    pub fn add(&self, src: SocketAddr, max_packet_size: MaxPacketSize, file_name: String, file_size: u64, reader: BufReader<File>, congestion_cache: Arc<CongestionCache>, current_src_addr: SocketAddr) -> ConnectionId {
        let mut guard = self.map.write().expect("failed to lock");
        let connection_id = Self::generate_connection_id(&*guard);
        let state = ConnectionState::new(connection_id, src, max_packet_size, file_name, file_size, reader, congestion_cache, current_src_addr);
